document.addEventListener('DOMContentLoaded', () => {
   // State variable to track the current input mode
   ...
   // Main calculation listener
   calculateButton.addEventListener('click', performCalculations);

   function setInputMode(mode) {
      if (currentInputMode === mode) return; // Do nothing if already active
   ...
   function performCalculations() {
      const inputs = {
         ...
         'DEF': parseFloat(document.getElementById('def').value) || 0,
      ...
      const RAR = battingRuns + baserunningRuns + inputs.DEF + posAdjustment + replacementRuns;
      const RUNS_PER_WIN = 10;
      const fWAR = RAR / RUNS_PER_WIN;
      ...
      document.getElementById('war-result').textContent = fWAR.toFixed(1);
   }
});